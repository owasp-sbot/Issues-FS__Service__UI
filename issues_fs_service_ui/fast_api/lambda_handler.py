...
from issues_fs_service_ui.config import LAMBDA_DEPENDENCIES__ISSUES_FS_SERVICE_UI
...
if os.getenv('AWS_REGION'):  # only execute if we are not running inside an AWS Lambda function
    ...
    load_dependencies(LAMBDA_DEPENDENCIES__ISSUES_FS_SERVICE_UI)
...
try:
    ...
    from issues_fs_service_ui.fast_api.Issues_FS__Service_UI__Fast_API import Issues_FS__Service_UI__Fast_API

    with Issues_FS__Service_UI__Fast_API() as _:
        _.setup()
        handler = _.handler()
        app     = _.app()
